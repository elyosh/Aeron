...
static int skyline_pack_anim(const AnimImage* anim, const int* order, int n, int atlas_w, int* out_ax,
							 int* out_ay) {
...
bool atlas_pack_compute(const AnimImage* anim, AtlasPack* out) {
	...
	for (int i = 0; i < anim->count; i++) {
		...
		if (img->width > max_w)
			max_w = img->width;
		total_area +=
			(long)(img->width + 2 * IMGBAKE_ATLAS_GUTTER) * (long)(img->height + 2 * IMGBAKE_ATLAS_GUTTER);
		valid++;
	}
	if (valid == 0 || max_w <= 0)
		return false;
	...
	for (int i = 1; i < n_order; i++) {
		...
		while (j >= 0) {
			...
			bool          key_bigger = (kkey > kj_key) || (kkey == kj_key && akey > aj);
			if (!key_bigger)
				break;
			order[j + 1] = order[j];
			j--;
	...
	for (int wi = 0; wi < n_widths; wi++) {
		...
		double aspect = (try_w >= h) ? (double)try_w / (double)h : (double)h / (double)try_w;
...
void atlas_pack_post_dims(const AtlasPack* pack, bool scale, bool svga_mode, int* out_w, int* out_h) {
	if (!pack || !out_w || !out_h)
		return;
...
bool atlas_emit_yaml(const char* yaml_path, const AnimImage* anim, const AtlasPack* pack, int post_atlas_w,
					 int post_atlas_h, bool scale, bool svga_mode, char* err, size_t errsz) {
	...
	for (int i = 0; i < anim->count; i++) {
		...
		if (fprintf(yf,
					...
					yaml_x, yaml_y, yaml_w, yaml_h, img->pixels ? img->left : 0,
