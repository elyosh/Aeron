...
int scale_y_4k(int v) { return (int)((v * 54 + 2) / 5); }

int scale_svga_xy_to_4k(int v) { return (v * 9 + 1) / 2; }
...
static bool lanczos_axis_resample(const uint8_t* src, int sw, int sh, uint8_t* dst, int dw, int dh, int by,
								  bool axis_y) {
	...
	for (int j = 0; j < N_dst; j++) {
		...
		for (int i = 0; i < N_other; i++) {
			...
			b *= iw;
			a *= iw;
			uint8_t* q = axis_y ? dst + ((size_t)j * dw + i) * 4  /* row j, col i */
...
bool atlas_vga_to_4k(uint8_t** inout_pixels, int* inout_w, int* inout_h) {
	...
	for (int y = 0; y < fh; y++) {
		...
		for (int x = 0; x < sw; x++) {
			double r = 0, g = 0, b = 0, a = 0;
			for (int t = 0; t < n_unique; t++) {
				const uint8_t* p  = src + ((size_t)src_rows[t] * sw + x) * 4;
				double         wt = aggw[t];
				r += p[0] * wt;
				g += p[1] * wt;
				b += p[2] * wt;
	...
	for (int y = 0; y < fh; y++) {
		const uint32_t* vrow = (const uint32_t*)(vmid + (size_t)y * sw * 4);
