...
bool font_atlas_load(const char* basename, FontAtlasResult* r, char* err, size_t err_size) {
	...
	FontAtlasGlyph* glyphs = (FontAtlasGlyph*)calloc((size_t)count, sizeof(FontAtlasGlyph));
	if (!glyphs) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (glyphs[i].advance > max_advance)
			max_advance = glyphs[i].advance;
	}
	free(fnt);
	...
	if (png_w != atlas_w || png_h != atlas_h) {
		set_err(err, err_size, "%s: PNG %dx%d != .fnt %dx%d", path, png_w, png_h, atlas_w, atlas_h);
		free(rgba);
		free(glyphs);
		return false;
	}
	...
	for (int g = 0; g < count; g++) {
		...
		if (gw <= 0 || gh <= 0 || gx + gw > atlas_w || gy + gh > atlas_h)
			continue;
		int min_x = gw, max_x = -1;
		for (int x = 0; x < gw; x++) {
			for (int y = 0; y < gh; y++) {
				uint8_t a = rgba[((size_t)(gy + y) * (size_t)atlas_w + (size_t)(gx + x)) * 4u + 3u];
				if (a >= 32) {
					if (x < min_x)
						min_x = x;
					if (x > max_x)
