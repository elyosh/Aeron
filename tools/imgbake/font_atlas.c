...
static void morpho_pass(const uint8_t* src, uint8_t* dst, int w, int h, bool dilate) {
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
...
bool font_atlas_build(const uint8_t* ttf_data, size_t ttf_size, const FontAtlasParams* p, FontAtlasResult* r,
	...
	int baseline = p->baseline >= 0 ? p->baseline : (int)(asc_fu * scale + 0.5f);
	...
	for (int g = 0; g < num_chars; g++) {
		...
		stbtt_GetGlyphBitmapBox(&info, gi[g].gidx, gi[g].scale_x, scale, &gi[g].x0, &gi[g].y0, &gi[g].x1,
								&gi[g].y1);

		int adv_px = (int)(gi[g].adv_fu * gi[g].scale_x + 0.5f);
		if (adv_px > max_advance_px)
			max_advance_px = adv_px;
		...
		if (eff_x1 > max_glyph_right)
			max_glyph_right = eff_x1;
		if (eff_x0 < max_glyph_left)
			max_glyph_left = eff_x0;
	}

	int cell_w_auto = max_advance_px;
	if (max_glyph_right > cell_w_auto)
		cell_w_auto = max_glyph_right;
	...
	if (atlas_w > 0xFFFF || atlas_h > 0xFFFF) {
		set_err(err, err_size, "atlas exceeds 16-bit dim: %dx%d", atlas_w, atlas_h);
		free(gi);
		return false;
	}

	uint8_t*        rgba       = (uint8_t*)calloc((size_t)atlas_w * (size_t)atlas_h * 4u, 1);
	FontAtlasGlyph* glyphs     = (FontAtlasGlyph*)calloc((size_t)num_chars, sizeof(FontAtlasGlyph));
	...
	for (int g = 0; g < num_chars; g++) {
		...
		if (gw > 0) {
			if (cp >= 0 && cp < 256 && p->compression_glyph_lsb_atlas[cp] >= 0)
		...
		if (advance_baked < 0)
			advance_baked = 0;
		if (advance_baked > 0xFFFF)
			advance_baked = 0xFFFF;
		...
		stbtt_MakeGlyphBitmap(&info, scratch + (size_t)pad * (size_t)eff_w + (size_t)pad, gw, gh, eff_w,
		...
		if (pad > 0 && gi[g].y1 <= 0) {
			...
			if (baseline_eff_y < eff_h) {
				...
				for (int y = baseline_eff_y; y < eff_h; y++)
					memset(writable + (size_t)y * (size_t)eff_w, 0, (size_t)eff_w);
			}
		}
		...
		for (int y = 0; y < rh; y++) {
			int            sy   = (ry - dst_y) + y;
			const uint8_t* srow = cur_scratch + (size_t)sy * (size_t)eff_w + (size_t)(rx - dst_x);
			uint8_t*       drow = rgba + ((size_t)(ry + y) * (size_t)atlas_w + (size_t)rx) * 4u;
			for (int x = 0; x < rw; x++) {
				uint8_t a = srow[x];
				drow[0]   = a;
				drow[1]   = a;
	...
	r->cap_height_atlas  = cap_height_fu > 0 ? (int)(cap_height_fu * scale + 0.5f) : r->ascent_atlas;
	...
	r->ascender_clip_px  = r->ascent_atlas > baseline ? r->ascent_atlas - baseline : 0;
	r->descender_clip_px =
		r->descent_atlas > (cell_h - baseline) ? r->descent_atlas - (cell_h - baseline) : 0;
	r->rgba       = rgba;
	r->glyphs     = glyphs;
...
bool font_atlas_write(const FontAtlasResult* r, const char* basename, char* err, size_t err_size) {
	...
	v = FNT_VERSION;
	memcpy(hdr + 4, &v, 2);
	v = (uint16_t)r->first_char;
	memcpy(hdr + 6, &v, 2);
	v = (uint16_t)r->num_chars;
	memcpy(hdr + 8, &v, 2);
	v = (uint16_t)r->atlas_w;
	memcpy(hdr + 10, &v, 2);
	v = (uint16_t)r->atlas_h;
	memcpy(hdr + 12, &v, 2);
	v = (uint16_t)r->cell_w;
	memcpy(hdr + 14, &v, 2);
	v = (uint16_t)r->cell_h;
	memcpy(hdr + 16, &v, 2);
	v = (uint16_t)r->baseline;
	memcpy(hdr + 18, &v, 2);
