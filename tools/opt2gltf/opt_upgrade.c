...
static uint16_t read_u16(const uint8_t* p) { return (uint16_t)p[0] | (uint16_t)((uint16_t)p[1] << 8); }
...
static size_t builder_reserve(OptImageBuilder* builder, size_t bytes, size_t alignment) {
	...
	if (aligned > UINT32_MAX - OPT_V2_BASE_ADDRESS || bytes > UINT32_MAX - OPT_V2_BASE_ADDRESS - aligned) {
...
static uint32_t builder_address(size_t offset) { return OPT_V2_BASE_ADDRESS + (uint32_t)offset; }

static uint32_t builder_blob(OptImageBuilder* builder, const void* data, size_t bytes, size_t alignment) {
	if (!bytes)
		return 0;
...
static uint32_t builder_node(OptImageBuilder* builder, const char* name, int32_t type, int32_t param1,
							 uint32_t param2, const uint32_t* children, int32_t child_count) {
	uint32_t name_address     = 0;
	uint32_t children_address = 0;
	if (name && name[0])
		name_address = builder_blob(builder, name, strlen(name) + 1, 1);
	if (child_count > 0)
		children_address = builder_blob(builder, children, (size_t)child_count * sizeof *children, 4);
...
static int validate_v1_graph(const uint8_t* source, size_t size, opt_error_t* error) {
	...
	while (stack_count) {
		...
		if (!address)
			continue;
		size_t i = 0;
		while (i < visited_count && visited[i] != address)
			++i;
		if (i != visited_count)
			continue;
...
static uint32_t build_data_node(OptImageBuilder* builder, int32_t type, const void* data, int32_t count,
								size_t stride) {
	const uint32_t payload = count > 0 ? builder_blob(builder, data, (size_t)count * stride, 4) : 0;
...
static int vec2_equal(opt_vec2_t left, opt_vec2_t right) { return left.u == right.u && left.v == right.v; }
...
static int canonicalize_vec3(OptImageBuilder* builder, const opt_vec3_t* source, int32_t source_count,
							 int32_t extra_capacity, opt_vec3_t** output, int32_t* output_count,
							 int32_t** index_map) {
	...
	opt_vec3_t*  values   = capacity ? (opt_vec3_t*)malloc(capacity * sizeof *values) : NULL;
	int32_t*     mapping  = source_count ? (int32_t*)malloc((size_t)source_count * sizeof *mapping) : NULL;
...
static int canonicalize_vec2(OptImageBuilder* builder, const opt_vec2_t* source, int32_t source_count,
							 opt_vec2_t** output, int32_t* output_count, int32_t** index_map) {
	if (source_count < 0 || (source_count && !source)) {
		builder_fail(builder, "invalid version-1 OPT texture-coordinate table");
		return 0;
	}
	opt_vec2_t* values  = source_count ? (opt_vec2_t*)malloc((size_t)source_count * sizeof *values) : NULL;
	int32_t*    mapping = source_count ? (int32_t*)malloc((size_t)source_count * sizeof *mapping) : NULL;
...
static void append_vertex_extrema(OptCanonicalMesh* canonical) {
	...
	for (int32_t index = 1; index < canonical->vertex_count; ++index) {
		...
		if (value.y < minimum.y)
			minimum.y = value.y;
		if (value.z < minimum.z)
			minimum.z = value.z;
		if (value.x > maximum.x)
			maximum.x = value.x;
		if (value.y > maximum.y)
			maximum.y = value.y;
		if (value.z > maximum.z)
			maximum.z = value.z;
	}
...
static int canonical_mesh_build(OptImageBuilder* builder, const opt_mesh_t* source,
	...
	if (!canonicalize_vec3(builder, source->vertices, source->vertex_count, 2, &canonical->vertices,
						   &canonical->vertex_count, &canonical->vertex_map) ||
		!canonicalize_vec3(builder, source->normals, source->normal_count, 0, &canonical->normals,
						   &canonical->normal_count, &canonical->normal_map) ||
		!canonicalize_vec2(builder, source->uvs, source->uv_count, &canonical->uvs, &canonical->uv_count,
						   &canonical->uv_map)) {
...
static int32_t remap_index(OptImageBuilder* builder, int32_t index, const int32_t* mapping,
						   int32_t source_count, const char* kind) {
	if (index == -1)
		return -1;
...
static uint32_t build_face_node(OptImageBuilder* builder, const opt_face_group_t* group,
								const opt_mesh_t* source_mesh, const OptCanonicalMesh* canonical) {
	if (group->face_count < 0 || group->edges_count < 0 || (group->face_count && !group->faces)) {
	...
	for (size_t face = 0; face < face_count; ++face) {
		...
		for (int corner = 0; corner < 4; ++corner)
			indices[corner] = remap_index(builder, input->verts[corner], canonical->vertex_map,
										  source_mesh->vertex_count, "vertex");
		memcpy(indices + 4, input->edges, 4 * sizeof(int32_t));
		for (int corner = 0; corner < 4; ++corner) {
			indices[8 + corner]  = remap_index(builder, input->uvs[corner], canonical->uv_map,
											   source_mesh->uv_count, "texture coordinate");
			indices[12 + corner] = remap_index(builder, input->normals[corner], canonical->normal_map,
											   source_mesh->normal_count, "normal");
		}
		if (builder->failed)
			return 0;
	...
	return builder_node(builder, NULL, OPT_NODE_FACE_DATA, group->face_count, builder_address(offset), NULL,
						0);
}

static uint32_t build_lod_node(OptImageBuilder* builder, const opt_lod_t* lod, const uint32_t* texture_nodes,
							   int32_t texture_count, const opt_mesh_t* source_mesh,
		...
		if (group->state_count > 1) {
			...
			if (!builder->failed)
				children[child_count++] =
					builder_node(builder, NULL, OPT_NODE_NODE_SWITCH, 0, 0, states, group->state_count);
		} else if (group->texture_index >= 0) {
			if (group->texture_index >= texture_count) {
				builder_fail(builder, "invalid version-1 OPT texture index %d", group->texture_index);
				break;
			}
	...
	const uint32_t result =
		builder->failed ? 0 : builder_node(builder, NULL, OPT_NODE_GROUP, 1, 0, children, child_count);
	free(children);
	return result;
}
...
static uint32_t build_face_group_node(OptImageBuilder* builder, const opt_mesh_t* mesh,
	...
	uint32_t* children =
		mesh->lod_count ? (uint32_t*)malloc((size_t)mesh->lod_count * sizeof *children) : NULL;
	float* thresholds = mesh->lod_count ? (float*)malloc((size_t)mesh->lod_count * sizeof *thresholds) : NULL;
	...
	for (int32_t lod = 0; lod < mesh->lod_count; ++lod) {
		...
		children[lod] =
			build_lod_node(builder, &mesh->lods[lod], texture_nodes, texture_count, mesh, canonical);
	}
	const uint32_t payload =
		mesh->lod_count ? builder_blob(builder, thresholds, (size_t)mesh->lod_count * sizeof *thresholds, 4)
						: 0;
	const uint32_t result = builder->failed
								? 0
								: builder_node(builder, NULL, OPT_NODE_FACE_GROUPING, mesh->lod_count,
											   payload, children, mesh->lod_count);
	free(children);
	free(thresholds);
	return result;
}

static uint32_t build_descriptor_node(OptImageBuilder* builder, const opt_mesh_descriptor_t* descriptor) {
	uint8_t payload[72];
...
static uint32_t build_mesh_root(OptImageBuilder* builder, const opt_mesh_t* mesh,
								const uint32_t* texture_nodes, int32_t texture_count) {
	if (mesh->hardpoint_count < 0 || (mesh->hardpoint_count && !mesh->hardpoints) || mesh->vertex_count < 0 ||
		(mesh->vertex_count && !mesh->vertices) || mesh->normal_count < 0 ||
		(mesh->normal_count && !mesh->normals) || mesh->uv_count < 0 || (mesh->uv_count && !mesh->uvs)) {
		builder_fail(builder, "invalid version-1 OPT mesh data");
		return 0;
	}
	OptCanonicalMesh canonical;
	if (!canonical_mesh_build(builder, mesh, &canonical))
		return 0;
	const size_t child_capacity = 6u + (size_t)(mesh->hardpoint_count > 0 ? mesh->hardpoint_count : 0);
	...
	if (mesh->has_rotation_scale) {
		...
		children[count++]      = builder_node(builder, NULL, OPT_NODE_ROTATION_SCALE, 1, payload, NULL, 0);
	}
	for (int32_t index = 0; index < mesh->hardpoint_count; ++index) {
		...
		const uint32_t address = builder_blob(builder, payload, sizeof payload, 4);
		children[count++]      = builder_node(builder, NULL, OPT_NODE_HARDPOINT, 1, address, NULL, 0);
	}
	children[count++] = build_data_node(builder, OPT_NODE_MESH_VERTICES, canonical.vertices,
										canonical.vertex_count, sizeof(opt_vec3_t));
	children[count++] = build_data_node(builder, OPT_NODE_VERTEX_NORMALS, canonical.normals,
										canonical.normal_count, sizeof(opt_vec3_t));
	children[count++] = build_data_node(builder, OPT_NODE_TEXTURE_COORDINATES, canonical.uvs,
										canonical.uv_count, sizeof(opt_vec2_t));
	children[count++] = build_face_group_node(builder, mesh, texture_nodes, texture_count, &canonical);
	const uint32_t result =
		builder->failed ? 0 : builder_node(builder, NULL, OPT_NODE_GROUP, 1, 0, children, count);
	free(children);
	canonical_mesh_free(&canonical);
	return result;
}

static int build_texture_nodes(OptImageBuilder* builder, const opt_file_t* model, uint32_t* texture_nodes) {
	uint32_t* palettes =
		model->texture_count ? (uint32_t*)calloc((size_t)model->texture_count, sizeof *palettes) : NULL;
	...
	for (int32_t index = 0; index < model->texture_count; ++index) {
		...
		int32_t shared = 0;
		while (shared < index &&
			   memcmp(texture->native_shade_table, model->textures[shared].native_shade_table,
					  OPT_NATIVE_SHADE_TABLE_BYTES) != 0)
			++shared;
		palettes[index] = shared < index ? palettes[shared]
										 : builder_blob(builder, texture->native_shade_table,
														OPT_NATIVE_SHADE_TABLE_BYTES, 4);
	}
	for (int32_t index = 0; index < model->texture_count && !builder->failed; ++index) {
		const opt_texture_t* texture = &model->textures[index];
		if (texture->width <= 0 || texture->height <= 0 || texture->mip_chain_bytes <= 0 ||
			!texture->pixels) {
		...
		write_u32(payload + 8, texture->mip_count > 1 ? (uint32_t)(texture->width * texture->height) : 0);
		...
		texture_nodes[index] =
			builder_node(builder, texture->name, OPT_NODE_TEXTURE, 1, builder_address(offset), NULL, 0);
	}
	free(palettes);
	return !builder->failed;
}

int opt_upgrade_v1_memory(const void* source, size_t source_size, uint8_t** output, size_t* output_size,
						  opt_error_t* error) {
	if (output)
		*output = NULL;
	if (output_size)
	...
	if (!validate_v1_graph((const uint8_t*)source, source_size, error))
		return 0;
	...
	const size_t    root_table = builder_reserve(&builder, (size_t)model->mesh_count * sizeof(uint32_t), 4);
	uint32_t*       texture_nodes =
		model->texture_count ? (uint32_t*)calloc((size_t)model->texture_count, sizeof *texture_nodes) : NULL;
	...
	for (int32_t mesh = 0; mesh < model->mesh_count && !builder.failed; ++mesh) {
		const uint32_t root =
			build_mesh_root(&builder, &model->meshes[mesh], texture_nodes, model->texture_count);
