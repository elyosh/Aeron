...
static int convert_one(const char* opt_path, const char* out_dir, const char* basename,
	...
	int ok = opt2gltf_convert(opt, out_dir, basename, smooth_angle_deg, repair_normals, emissive) ? 1 : 0;
	opt_free(opt);
	return ok;
}
...
int main(int argc, char** argv) {
	...
	if (S_ISDIR(st.st_mode)) {
		...
		while ((de = readdir(d))) {
			if (de->d_name[0] == '.')
				continue;
			if (!ends_with_icase(de->d_name, ".opt"))
				continue;
			char src[1024];
			...
			if (convert_one(src, out_sub, base, smooth_angle_deg, repair_normals, emissive))
				++ok_count;
			else
				++fail_count;
		}
	...
	return convert_one(in, outd, base, smooth_angle_deg, repair_normals, emissive) ? 0 : 1;
}
