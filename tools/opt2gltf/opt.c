...
static int range_ok(const opt_ctx_t* c, size_t off, size_t len) { return off + len <= c->size; }
...
static int read_node(const opt_ctx_t* c, size_t off, opt_node_t* out) {
	if (!range_ok(c, off, OPT_NODE_HEADER_BYTES))
		return 0;
	out->off             = off;
...
static void for_each_child(opt_ctx_t* c, const opt_node_t* n, child_fn_t fn, void* user) {
	if (n->child_count <= 0 || n->child_table_ptr == 0)
		return;
	size_t table = ptr_to_off(c, n->child_table_ptr);
	for (int32_t i = 0; i < n->child_count; i++) {
		if (!range_ok(c, table + (size_t)i * 4, 4))
			return;
		int32_t cptr = ru32(c->buf, table + (size_t)i * 4);
		if (cptr == 0)
			continue;
		opt_node_t child;
		if (!read_node(c, ptr_to_off(c, cptr), &child))
			continue;
		fn(c, &child, user);
		if (c->failed)
			return;
...
static void parse_mesh_vertices(opt_ctx_t* c, const opt_node_t* n, opt_mesh_t* m) {
	...
	if (count <= 0 || off == 0)
		return;
	if (!range_ok(c, off, (size_t)count * OPT_VEC3_BYTES))
		return;

	m->vertices = (opt_vec3_t*)calloc((size_t)count, sizeof *m->vertices);
	if (!m->vertices) {
		set_err(c, "out of memory (vertices)");
		return;
	}
...
static void parse_vertex_normals(opt_ctx_t* c, const opt_node_t* n, opt_mesh_t* m) {
	...
	if (!m->normals) {
		set_err(c, "out of memory (normals)");
		return;
	}
...
static void parse_texcoords(opt_ctx_t* c, const opt_node_t* n, opt_mesh_t* m) {
	...
	if (count <= 0 || off == 0)
		return;
	if (!range_ok(c, off, (size_t)count * OPT_VEC2_BYTES))
		return;

	m->uvs = (opt_vec2_t*)calloc((size_t)count, sizeof *m->uvs);
	if (!m->uvs) {
		set_err(c, "out of memory (uvs)");
		return;
	}
	m->uv_count = count;
	for (int32_t i = 0; i < count; i++) {
...
static void parse_mesh_descriptor(opt_ctx_t* c, const opt_node_t* n, opt_mesh_t* m) {
	size_t off = ptr_to_off(c, n->p2);
	if (off == 0 || !range_ok(c, off, OPT_MESH_DESCRIPTOR_BYTES))
		return;
	...
	rvec3(c->buf, off + 8 + 4 * OPT_VEC3_BYTES + sizeof(int32_t), &d->target);
	m->has_descriptor = 1;
}

static void parse_hardpoint(opt_ctx_t* c, const opt_node_t* n, opt_mesh_t* m) {
	size_t off = ptr_to_off(c, n->p2);
	if (off == 0 || !range_ok(c, off, OPT_HARDPOINT_BYTES))
		return;

	opt_hardpoint_t* grown =
		(opt_hardpoint_t*)realloc(m->hardpoints, (size_t)(m->hardpoint_count + 1) * sizeof *grown);
	if (!grown) {
		set_err(c, "out of memory (hardpoints)");
		return;
	}
	m->hardpoints       = grown;
...
static void parse_engine_glow(opt_ctx_t* c, const opt_node_t* n, opt_mesh_t* m) {
	size_t off = ptr_to_off(c, n->p2);
	if (off == 0 || !range_ok(c, off, OPT_ENGINE_GLOW_BYTES))
		return;

	opt_engine_glow_t* grown =
		(opt_engine_glow_t*)realloc(m->engine_glows, (size_t)(m->engine_glow_count + 1) * sizeof *grown);
	if (!grown) {
		set_err(c, "out of memory (engine glows)");
		return;
	}
...
static void parse_rotation_scale(opt_ctx_t* c, const opt_node_t* n, opt_mesh_t* m) {
	size_t off = ptr_to_off(c, n->p2);
	if (off == 0 || !range_ok(c, off, OPT_ROTATION_SCALE_BYTES))
		return;
...
static int32_t texture_lookup_or_add(opt_ctx_t* c, const char* name) {
	...
	if (c->texture_count == c->texture_capacity) {
		...
		opt_texture_t* grown   = (opt_texture_t*)realloc(c->textures, (size_t)new_cap * sizeof *grown);
		if (!grown) {
			set_err(c, "out of memory (textures)");
			return -1;
		}
		memset(grown + c->texture_capacity, 0, (size_t)(new_cap - c->texture_capacity) * sizeof *grown);
		c->textures         = grown;
...
static int32_t count_mip_levels(int32_t width, int32_t height, int32_t chain_bytes) {
	...
	while (accumulated < chain_bytes) {
		...
		if (accumulated + level_bytes > chain_bytes)
			break;
...
static void parse_texture_data(opt_ctx_t* c, const opt_node_t* n, int32_t tex_index) {
	...
	if (data_off == 0 || !range_ok(c, data_off, OPT_TEX_DATA_HEADER_BYTES))
		return;
	...
	int32_t chain_bytes = (texture_size == 0) ? (int32_t)((size_t)width * (size_t)height) : data_size;
	if (t->pixels == NULL && chain_bytes > 0 && range_ok(c, pixels_off, (size_t)chain_bytes)) {
		t->pixels = (uint8_t*)malloc((size_t)chain_bytes);
		if (!t->pixels) {
			set_err(c, "out of memory (texture pixels)");
			return;
		}
	...
	size_t pal_off = (palette_type == 0) ? ptr_to_off(c, palette_ptr) : pixels_off + (size_t)chain_bytes;
	if (pal_off != 0 && range_ok(c, pal_off, OPT_PALETTE_BYTES)) {
		memcpy(t->palette, c->buf + pal_off, OPT_PALETTE_BYTES);
	}
	if (c->version == 1 && pal_off != 0 && range_ok(c, pal_off, OPT_NATIVE_SHADE_TABLE_BYTES)) {
		...
		memcpy(t->native_shade_table, c->buf + pal_off, OPT_NATIVE_SHADE_TABLE_BYTES);
	}
	...
	if (t->alpha == NULL && n->child_count > 0 && n->child_table_ptr != 0) {
		...
		for (int32_t i = 0; i < n->child_count; i++) {
			...
			if (!read_node(c, ptr_to_off(c, cptr), &child))
				continue;
			if (child.type != OPT_NODE_TEXTURE_ALPHA)
				continue;
			int32_t alpha_bytes = child.p1;
			size_t  alpha_off   = ptr_to_off(c, child.p2);
			if (alpha_bytes <= 0 || alpha_off == 0 || !range_ok(c, alpha_off, (size_t)alpha_bytes))
				break;
			t->alpha = (uint8_t*)malloc((size_t)alpha_bytes);
			if (!t->alpha) {
				set_err(c, "out of memory (texture alpha)");
				return;
			}
...
static void parse_face_data(opt_ctx_t* c, const opt_node_t* n, opt_lod_builder_t* lb) {
	...
	if (lb->group_count == lb->group_capacity) {
		...
		opt_face_group_t* grown   = (opt_face_group_t*)realloc(lb->groups, (size_t)new_cap * sizeof *grown);
		if (!grown) {
			set_err(c, "out of memory (face groups)");
			return;
		}
		lb->groups         = grown;
	...
	if (fg->state_count > 1) {
		...
		memcpy(fg->state_textures, lb->current_states, (size_t)fg->state_count * sizeof(int32_t));
	}
	...
	for (int32_t i = 0; i < face_count; i++) {
		...
		for (int k = 0; k < 4; k++)
			f->verts[k] = ru32(c->buf, ix + (size_t)(0 + k) * 4);
		for (int k = 0; k < 4; k++)
			f->edges[k] = ru32(c->buf, ix + (size_t)(4 + k) * 4);
		for (int k = 0; k < 4; k++)
			f->uvs[k] = ru32(c->buf, ix + (size_t)(8 + k) * 4);
		for (int k = 0; k < 4; k++)
			f->normals[k] = ru32(c->buf, ix + (size_t)(12 + k) * 4);
		rvec3(c->buf, nrm_off + (size_t)i * OPT_VEC3_BYTES, &f->face_normal);
		rvec3(c->buf, tex_off + (size_t)i * 2 * OPT_VEC3_BYTES + 0 * OPT_VEC3_BYTES, &f->tex_direction);
		rvec3(c->buf, tex_off + (size_t)i * 2 * OPT_VEC3_BYTES + 1 * OPT_VEC3_BYTES, &f->tex_magnitude);
	}
}
...
static void parse_lod_node(opt_ctx_t* c, const opt_node_t* lod_group_node, float distance,
						   const opt_texture_binding_t* entry_binding, opt_texture_binding_t* exit_binding,
						   opt_lod_t* out) {
	...
	if (lb.pending_binding && lb.group_count > 0 && lb.current_texture >= 0) {
...
static void parse_face_grouping(opt_ctx_t* c, const opt_node_t* fg, opt_mesh_t* mesh) {
	int32_t lod_count = fg->p1;
	size_t  dist_off  = ptr_to_off(c, fg->p2);
	if (lod_count <= 0 || dist_off == 0)
		return;
	if (!range_ok(c, dist_off, (size_t)lod_count * sizeof(float)))
		return;

	mesh->lods = (opt_lod_t*)calloc((size_t)lod_count, sizeof *mesh->lods);
	if (!mesh->lods) {
		set_err(c, "out of memory (lods)");
		return;
	}
		...
		if (i < fg->child_count && range_ok(c, table + (size_t)i * 4, 4)) {
			...
			if (cptr != 0) {
				...
				if (read_node(c, ptr_to_off(c, cptr), &lod_node)) {
					...
					parse_lod_node(c, &lod_node, dist, &entry_binding, &exit_binding, &mesh->lods[i]);
					if (i == 0) {
...
static void on_mesh_descendant(opt_ctx_t* c, const opt_node_t* n, void* user) {
	...
	switch (n->type) {
		case OPT_NODE_MESH_VERTICES:
			parse_mesh_vertices(c, n, m);
			break;
		case OPT_NODE_VERTEX_NORMALS:
			parse_vertex_normals(c, n, m);
			break;
		case OPT_NODE_TEXTURE_COORDINATES:
			parse_texcoords(c, n, m);
			break;
		case OPT_NODE_MESH_DESCRIPTOR:
			parse_mesh_descriptor(c, n, m);
			break;
		case OPT_NODE_HARDPOINT:
			parse_hardpoint(c, n, m);
			break;
		case OPT_NODE_ENGINE_GLOW:
			parse_engine_glow(c, n, m);
			break;
		case OPT_NODE_ROTATION_SCALE:
			parse_rotation_scale(c, n, m);
			break;
		case OPT_NODE_FACE_GROUPING:
			parse_face_grouping(c, n, m);
			break;
		case OPT_NODE_GROUP:
...
static opt_file_t* parse_from_buffer(const uint8_t* data, size_t size, opt_error_t* err) {
	if (size < OPT_HEADER_BYTES_LEGACY + OPT_CONTAINER_BYTES) {
		if (err)
			snprintf(err->msg, sizeof err->msg, "file too small (%zu bytes)", size);
		return NULL;
	}
	...
	if (size - hdr_bytes != size_in_header) {
		if (err)
			snprintf(err->msg, sizeof err->msg, "size mismatch: file=%zu header_expects=%zu", size,
					 size_in_header + hdr_bytes);
		return NULL;
	}
	...
	int32_t root_table_p = ru32(data, cont + sizeof(int32_t) + OPT_CONTAINER_PAD_BYTES + sizeof(int32_t));
	...
	if (!meshes) {
		if (err)
			snprintf(err->msg, sizeof err->msg, "out of memory (meshes)");
		return NULL;
	}
	...
	for (int32_t i = 0; i < root_count; i++) {
		...
		if (cptr == 0)
			continue;
		opt_node_t top;
		if (!read_node(&c, ptr_to_off(&c, cptr), &top))
			continue;
		if (top.type != OPT_NODE_GROUP && top.type != OPT_NODE_TEXTURE)
			continue;
	...
	if (!f) {
		if (err)
			snprintf(err->msg, sizeof err->msg, "out of memory (file)");
		goto fail;
	}
	f->version       = version;
...
opt_file_t* opt_load_file(const char* path, opt_error_t* err) {
	...
	if (size <= 0) {
		fclose(fp);
		if (err)
			snprintf(err->msg, sizeof err->msg, "empty file '%s'", path);
		return NULL;
	}
	...
	if (fread(buf, 1, (size_t)size, fp) != (size_t)size) {
		...
		fclose(fp);
		if (err)
			snprintf(err->msg, sizeof err->msg, "short read on '%s'", path);
		return NULL;
	}
	fclose(fp);
...
const char* opt_mesh_type_name(opt_mesh_type_t t) {
	switch (t) {
		case OPT_MT_DEFAULT:
			return "Default";
		case OPT_MT_MAIN_HULL:
			return "MainHull";
		case OPT_MT_WING:
			return "Wing";
		case OPT_MT_FUSELAGE:
			return "Fuselage";
		case OPT_MT_GUN_TURRET:
			return "GunTurret";
		case OPT_MT_SMALL_GUN:
			return "SmallGun";
		case OPT_MT_ENGINE:
			return "Engine";
		case OPT_MT_BRIDGE:
			return "Bridge";
		case OPT_MT_SHIELD_GEN:
			return "ShieldGen";
		case OPT_MT_ENERGY_GEN:
			return "EnergyGen";
		case OPT_MT_LAUNCHER:
			return "Launcher";
		case OPT_MT_COMM_SYS:
			return "CommSys";
		case OPT_MT_BEAM_SYS:
			return "BeamSys";
		case OPT_MT_COMMAND_BEAM:
			return "CommandBeam";
		case OPT_MT_DOCKING_PLAT:
			return "DockingPlat";
		case OPT_MT_LANDING_PLAT:
			return "LandingPlat";
		case OPT_MT_HANGAR:
			return "Hangar";
		case OPT_MT_CARGO_POD:
			return "CargoPod";
		case OPT_MT_MISC_HULL:
			return "MiscHull";
		case OPT_MT_ANTENNA:
			return "Antenna";
		...
		case OPT_MT_ROTARY_GUN_TURRET:
			return "RotaryGunTurret";
		case OPT_MT_ROTARY_LAUNCHER:
			return "RotaryLauncher";
		case OPT_MT_ROTARY_COMM_SYS:
			return "RotaryCommSys";
		case OPT_MT_ROTARY_BEAM_SYS:
			return "RotaryBeamSys";
		case OPT_MT_ROTARY_COMMAND_BEAM:
			return "RotaryCommandBeam";
		case OPT_MT_HATCH:
			return "Hatch";
		case OPT_MT_CUSTOM:
			return "Custom";
		case OPT_MT_WEAPON_SYS_1:
			return "WeaponSys1";
		case OPT_MT_WEAPON_SYS_2:
			return "WeaponSys2";
		case OPT_MT_POWER_REGENERATOR:
			return "PowerRegenerator";
		case OPT_MT_REACTOR:
			return "Reactor";
	}
	return "?";
...
const char* opt_hardpoint_type_name(opt_hardpoint_type_t t) {
	switch (t) {
		case OPT_HP_NONE:
			return "None";
		case OPT_HP_LASER_CANNON:
			return "LaserCannon";
		case OPT_HP_ION_CANNON:
			return "IonCannon";
		case OPT_HP_TURBO_LASER:
			return "TurboLaser";
		case OPT_HP_ION_TURBO_LASER:
			return "IonTurboLaser";
		case OPT_HP_CLUSTER_MISSILE:
			return "ClusterMissile";
		case OPT_HP_TORPEDO_MAG_PULSE:
			return "TorpedoMagPulse";
		case OPT_HP_CONCUSSION_MISSILE:
			return "ConcussionMissile";
		case OPT_HP_PROTON_TORPEDO:
			return "ProtonTorpedo";
		case OPT_HP_ADV_CONCUSSION:
			return "AdvConcussion";
		case OPT_HP_ADV_PROTON_TORPEDO:
			return "AdvProtonTorpedo";
		case OPT_HP_ADV_TORPEDO_MAG_PULSE:
			return "AdvTorpedoMagPulse";
		case OPT_HP_BOMB:
			return "Bomb";
		case OPT_HP_BEAM_WEAPON:
			return "BeamWeapon";
		case OPT_HP_DUMB_BOMB:
			return "DumbBomb";
		case OPT_HP_FIRE_ROCKET:
			return "FireRocket";
	}
	return "?";
}
...
#define NEAR(a, b)                                                                                           \
	(fabsf((a).x - (b).x * OPT_Q15_UNIT) < tol && fabsf((a).y - (b).y * OPT_Q15_UNIT) < tol &&               \
	 fabsf((a).z - (b).z * OPT_Q15_UNIT) < tol)
	int pos = NEAR(rs->rotation_axis, za_pos) && NEAR(rs->direction_axis, ya) && NEAR(rs->up_axis, xa_pos);
	int neg = NEAR(rs->rotation_axis, za_neg) && NEAR(rs->direction_axis, ya) && NEAR(rs->up_axis, xa_neg);
#undef NEAR
	return pos || neg;
}

size_t opt_texture_mip_offset(const opt_texture_t* t, int level, int32_t* w, int32_t* h) {
...
int32_t opt_face_group_texture(const opt_face_group_t* g, int state) {
	...
	if (state < 0 || state >= g->state_count)
		return -1;
...
int opt_palette_classic_lit(const uint8_t palette[OPT_PALETTE_BYTES], int index) {
	...
	for (int plane = 1; plane <= 6; plane++) {
		uint16_t cp;
		memcpy(&cp, palette + ((size_t)plane * OPT_PALETTE_COLORS + (size_t)index) * OPT_PALETTE_BPP,
			   sizeof cp);
...
int opt_palette_index_emissive(const opt_file_t* opt, const uint8_t palette[OPT_PALETTE_BYTES], int index,
							   int tol) {
	int      base     = opt_palette_base_shade(opt);
	int      dim      = (opt && opt->version >= 5) ? OPT_XWA_DIM_SHADE : OPT_TIE_DIM_SHADE;
	size_t   base_off = (size_t)base * OPT_PALETTE_COLORS * OPT_PALETTE_BPP + (size_t)index * OPT_PALETTE_BPP;
	size_t   dim_off  = (size_t)dim * OPT_PALETTE_COLORS * OPT_PALETTE_BPP + (size_t)index * OPT_PALETTE_BPP;
	uint16_t cb, cd;
	...
	if (abs((int)rb - (int)rd) > tol)
		return 0;
	if (abs((int)gb - (int)gd) > tol)
		return 0;
	if (abs((int)bb - (int)bd) > tol)
		return 0;
	return 1;
}

void opt_palette_rgb(const uint8_t palette[OPT_PALETTE_BYTES], int shade, int index, uint8_t* r, uint8_t* g,
					 uint8_t* b) {
	size_t   off = (size_t)shade * OPT_PALETTE_COLORS * OPT_PALETTE_BPP + (size_t)index * OPT_PALETTE_BPP;
	uint16_t c;
