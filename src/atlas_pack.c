...
int Aeron_AtlasBlitRgba8(uint8_t* atlas, int atlas_width, int atlas_height, const uint8_t* source,
						 int source_width, int source_height, int x, int y, int gutter,
	...
	for (int row = -gutter; row < source_height + gutter; row++) {
		...
		uint8_t*       destination = atlas + ((size_t)(y + row) * atlas_width + (size_t)(x - gutter)) * 4u;
		for (int column = -gutter; column < 0; column++) {
			const int source_x = atlas_address_index(column, source_width, address_mode);
			memcpy(destination + (size_t)(column + gutter) * 4u, source_row + (size_t)source_x * 4u, 4u);
		}
...
static void atlas_sort_images(const AeronAtlasImage* images, int* order, int count) {
	...
	for (int i = 1; i < count; i++) {
		...
		while (j >= 0) {
			...
			const int other_side =
				images[other].width > images[other].height ? images[other].width : images[other].height;
...
static int atlas_plan_page(const AeronAtlasImage* images, const int* order, int first, int count,
						   const AeronAtlasBuildOptions* options, AeronAtlasRect** out_rects, int* out_width,
						   int* out_height) {
	uint64_t area   = 0;
...
static uint8_t* atlas_compose_page(const AeronAtlasImage* images, const AeronAtlasRect* rects, int count,
								   int width, int height, int gutter) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (!Aeron_AtlasBlitRgba8(rgba, width, height, image->rgba, image->width, image->height, rects[i].x,
								  rects[i].y, gutter, AERON_ATLAS_ADDRESS_CLAMP)) {
			free(rgba);
			return NULL;
		}
	}
	...
	while (first < count) {
		...
		if (atlas_plan_page(images, order, first, remaining, options, &rects, &width, &height)) {
			...
			while (low <= high) {
				...
				if (atlas_plan_page(images, order, first, candidate, options, &probe, &probe_width,
									&probe_height)) {
					free(rects);
					rects       = probe;
					width       = probe_width;
