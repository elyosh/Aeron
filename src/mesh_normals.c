...
static bool normals_error(AeronMeshNormalsError* error, int code, const char* format, ...) {
	if (error) {
		va_list args;
		error->code = code;
...
static const float* normal_position(const NormalContext* context, uint32_t index) {
	return (const float*)((const uint8_t*)context->input->positions +
						  (size_t)index * context->input->position_stride);
}

static NormalVec3 normal_face_normal(const NormalContext* context, const uint32_t vertices[3]) {
...
static float normal_corner_angle(const NormalContext* context, const uint32_t vertices[3], uint32_t corner) {
	...
	if (previous_length <= 1e-12 || next_length <= 1e-12)
		return 0.0f;
	cosine = (px * nx + py * ny + pz * nz) / (previous_length * next_length);
	if (cosine < -1.0)
		cosine = -1.0;
	if (cosine > 1.0)
		cosine = 1.0;
...
static bool normal_context_init(NormalContext* context, const AeronMeshNormalsInput* input,
	...
	context->incident_offsets = calloc((size_t)input->position_count + 1, sizeof *context->incident_offsets);
	context->visit_marks      = calloc(input->triangle_count, sizeof *context->visit_marks);
	context->stack            = malloc((size_t)input->triangle_count * sizeof *context->stack);
	if (!context->faces || !context->incident_offsets || !context->visit_marks || !context->stack)
		goto allocation_failed;

	for (uint32_t face_index = 0; face_index < input->triangle_count; ++face_index) {
		NormalFace* face = &context->faces[face_index];
		for (uint32_t corner = 0; corner < 3; ++corner) {
			const uint32_t vertex = input->triangle_position_indices[(size_t)face_index * 3 + corner];
			if (vertex >= input->position_count) {
				normal_context_release(context);
				return normals_error(error, 2, "triangle %u corner %u has position index %u, count is %u",
		...
		for (uint32_t corner = 0; corner < 3; ++corner)
			face->corner_angles[corner] = normal_corner_angle(context, face->vertices, corner);
	}
	for (uint32_t vertex = 0; vertex < input->position_count; ++vertex) {
		context->incident_offsets[vertex + 1] += context->incident_offsets[vertex];
	}
	incident_count     = context->incident_offsets[input->position_count];
	context->incidents = calloc(incident_count ? incident_count : 1, sizeof *context->incidents);
	cursor             = malloc((size_t)input->position_count * sizeof *cursor);
	if (!context->incidents || !cursor)
		goto allocation_failed;
	memcpy(cursor, context->incident_offsets, (size_t)input->position_count * sizeof *cursor);
	for (uint32_t face_index = 0; face_index < input->triangle_count; ++face_index) {
		...
		for (uint32_t corner = 0; corner < 3; ++corner) {
			...
			context->incidents[index] = (NormalIncident) { face_index, (uint8_t)corner };
		}
	}
	free(cursor);
	return true;
...
static bool normal_faces_share_edge(const NormalFace* lhs, uint32_t lhs_corner, const NormalFace* rhs,
									uint32_t rhs_corner) {
	...
	return lhs_previous == rhs_previous || lhs_previous == rhs_next || lhs_next == rhs_previous ||
		   lhs_next == rhs_next;
}
...
static NormalVec3 normal_sum_fan(const NormalContext* context, uint32_t begin, uint32_t end,
								 const NormalFace* reference, bool restrict_hemisphere, float smooth_cosine) {
	...
	for (uint32_t index = begin; index < end; ++index) {
		...
		if (context->visit_marks[incident->face] != context->visit_token)
			continue;
		if (restrict_hemisphere && normal_dot(face->normal, reference->normal) + 1e-6f < local_cosine)
			continue;
...
static NormalVec3 normal_build_corner(NormalContext* context, uint32_t reference_face, uint32_t vertex,
									  float smooth_cosine) {
	...
	while (stack_count) {
		...
		for (uint32_t index = begin; index < end; ++index) {
			...
			if (context->visit_marks[incident->face] == context->visit_token ||
				...
				!normal_faces_share_edge(current, current_corner, candidate, incident->corner) ||
				normal_dot(current->normal, candidate->normal) + 1e-6f < smooth_cosine)
				continue;
			context->visit_marks[incident->face] = context->visit_token;
			context->stack[stack_count++]        = incident->face;
		}
	}
	NormalVec3 result = normal_sum_fan(context, begin, end, reference, false, smooth_cosine);
	...
	if (normal_dot(result, reference->normal) <= 1e-4f) {
		NormalVec3 local = normal_sum_fan(context, begin, end, reference, true, smooth_cosine);
		result = normal_dot(local, reference->normal) > 1e-4f ? local
															  : normal_stable_face_normal(reference->normal);
	}
	return result;
}

bool Aeron_MeshNormalsBuildCorners(const AeronMeshNormalsInput* input, float* corner_normals,
								   AeronMeshNormalsError* error) {
	NormalContext context;
	if (error)
		memset(error, 0, sizeof *error);
	if (!input || !corner_normals || !input->positions || !input->triangle_position_indices ||
		input->position_count == 0 || input->triangle_count == 0 ||
		input->position_stride < 3 * sizeof(float) || !isfinite(input->smooth_angle_degrees) ||
		input->smooth_angle_degrees < 0.0f || input->smooth_angle_degrees > 180.0f)
	...
	if (!normal_context_init(&context, input, error))
		return false;
	const float smooth_cosine = cosf(input->smooth_angle_degrees * 3.14159265358979323846f / 180.0f);
	for (uint32_t face = 0; face < input->triangle_count; ++face) {
		for (uint32_t corner = 0; corner < 3; ++corner) {
			const NormalVec3 normal =
				normal_build_corner(&context, face, context.faces[face].vertices[corner], smooth_cosine);
