...
static bool map_format(uint32_t vk, AeronTextureFormat* out_fmt, bool* out_block, int* out_bpb) {
...
static Ktx2* ktx2_parse_bytes(const uint8_t* bytes, size_t blob_size, const char* src_label, bool own_bytes,
							  Ktx2OpenStatus* status) {
	const char* path = src_label ? src_label : "<mem>";
	if (status)
		*status = KTX2_OPEN_INVALID_DATA;
	if (!bytes)
		return NULL;
	...
	if ((faceCount != 1 && faceCount != 6) || (layerCount > 1) || (pixelDepth > 1)) {
	...
	if (faceCount == 6 && pixelWidth != pixelHeight) {
		ktx2_log("[ktx2] %s: cubemap requires square faces (%u x %u)", path, pixelWidth, pixelHeight);
		if (own_bytes)
			free(blob);
		return NULL;
	}
	...
	if (!levels || !level_owned) {
		if (status)
			*status = KTX2_OPEN_RESOURCE_FAILURE;
		free(levels);
		free(level_owned);
		if (own_bytes)
			free(blob);
	...
	for (uint32_t i = 0; i < levelCount; i++) {
		...
		if (off > blob_size || off + len > blob_size) {
			ktx2_log("[ktx2] %s: level %u index out of range "
					 ...
					 path, i, (unsigned long long)off, (unsigned long long)len, blob_size);
		...
		int lw = (int)(pixelWidth >> i);
		if (lw < 1)
			lw = 1;
		int lh = (int)(pixelHeight >> i);
		if (lh < 1)
			lh = 1;
		if (zstd_supercompressed) {
			...
			if (!out) {
				if (status)
					*status = KTX2_OPEN_RESOURCE_FAILURE;
				ktx2_log("[ktx2] %s: level %u alloc %llu failed", path, i, (unsigned long long)ulen);
				for (uint32_t j = 0; j < i; j++)
					free(level_owned[j]);
				free(level_owned);
				free(levels);
			...
			size_t got = ZSTD_decompress(out, (size_t)ulen, blob + off, (size_t)len);
			if (ZSTD_isError(got) || got != (size_t)ulen) {
				ktx2_log("[ktx2] %s: level %u zstd decompress failed (%s)", path, i,
...
Ktx2* ktx2_open_mem_status(const uint8_t* bytes, size_t size, const char* src_label, Ktx2OpenStatus* status) {
...
bool ktx2_is_block_compressed(const Ktx2* k) { return k && k->block_compressed; }
...
Ktx2Level ktx2_level_face(const Ktx2* k, int level, int face) {
	...
	if (!lvl.data || k->face_count <= 0 || face < 0 || face >= k->face_count)
		return (Ktx2Level) { .data = NULL, .size = 0, .width = 0, .height = 0 };
