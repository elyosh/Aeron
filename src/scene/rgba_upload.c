...
AeronTexture* aeron_scene_upload_rgba8(AeronCommandBuffer* cmd, const uint8_t* pixels, int width, int height,
									   size_t pitch, AeronTextureFormat format, AeronColorSpace color_space,
									   AeronImageAlphaMode alpha_mode, bool generate_mips,
									   const char* debug_name) {
	if (!cmd || !pixels || width <= 0 || height <= 0 || pitch < (size_t)width * 4 ||
		(size_t)height > SIZE_MAX / pitch)
		return NULL;
	const int               mip_count = rgba_mip_count(width, height, generate_mips);
	uint8_t**               levels    = calloc((size_t)mip_count, sizeof *levels);
	AeronTextureUploadDesc* uploads   = calloc((size_t)mip_count, sizeof *uploads);
	if (!levels || !uploads) {
		free(levels);
		free(uploads);
		return NULL;
