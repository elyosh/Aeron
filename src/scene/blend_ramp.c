...
void Aeron_BlendRampToggle(AeronBlendRamp* s) { s->target = (s->target >= 0.5f) ? 0.0f : 1.0f; }

void Aeron_BlendRampAdvance(AeronBlendRamp* s, int32_t delta_us, float effective_target) {
	if (effective_target < 0.0f)
		effective_target = 0.0f;
	if (effective_target > 1.0f)
		effective_target = 1.0f;
...
bool Aeron_BlendRampIsSolid(const AeronBlendRamp* s) {
	return s->alpha <= BLEND_SNAP_EPSILON || s->alpha >= 1.0f - BLEND_SNAP_EPSILON;
}
