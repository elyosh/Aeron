...
static int font_metrics_valid(const FontMetrics* metrics) {
	if (!metrics || !metrics->glyphs || metrics->atlas_w <= 0 || metrics->atlas_h <= 0 ||
		metrics->atlas_w > UINT16_MAX || metrics->atlas_h > UINT16_MAX || !metrics->glyph_count ||
		!metrics->cell_w || !metrics->cell_h || metrics->baseline > metrics->cell_h ||
	...
	for (uint16_t index = 0; index < metrics->glyph_count; ++index) {
		...
		if ((uint32_t)glyph->atlas_x + glyph->atlas_w > (uint32_t)metrics->atlas_w ||
			(uint32_t)glyph->atlas_y + glyph->atlas_h > (uint32_t)metrics->atlas_h)
			return 0;
	}
	return 1;
}

static void font_atlas_assign(AeronFontAtlas* out, AeronTexture* texture, FontMetrics* metrics) {
	out->texture    = texture;
...
static int font_metrics_parse(const uint8_t* fnt, size_t size, const char* label, FontMetrics* out) {
	...
	if (version != 2) {
		Aeron_LogError("aeron.scene", "font: '%s' unsupported version %u", label, version);
		return 0;
	}
...
int AeronFontAtlas_InitRgba8(AeronFontAtlas* out, AeronCommandBuffer* cmd,
							 const AeronFontAtlasRgba8Desc* desc) {
	if (!font_atlas_is_empty(out) || !cmd || !desc || !desc->pixels || !desc->glyphs ||
		desc->pitch < (size_t)desc->width * 4 || (size_t)desc->height > SIZE_MAX / desc->pitch ||
		(desc->alpha_mode != AERON_IMAGE_ALPHA_STRAIGHT &&
		 desc->alpha_mode != AERON_IMAGE_ALPHA_PREMULTIPLIED) ||
		(desc->format != AERON_TEXTURE_FORMAT_RGBA8_UNORM && desc->format != AERON_TEXTURE_FORMAT_RGBA8_SRGB))
		return 0;
	FontMetrics metrics = {
		.atlas_w     = desc->width,
		.atlas_h     = desc->height,
	...
	if (!font_metrics_valid(&metrics))
		return 0;
	AeronFontGlyph* glyphs = malloc((size_t)metrics.glyph_count * sizeof *glyphs);
	if (!glyphs)
		return 0;
	memcpy(glyphs, metrics.glyphs, (size_t)metrics.glyph_count * sizeof *glyphs);
	metrics.glyphs = glyphs;
	AeronTexture* texture =
		aeron_scene_upload_rgba8(cmd, desc->pixels, desc->width, desc->height, desc->pitch, desc->format,
								 desc->color_space, desc->alpha_mode, desc->generate_mips, desc->debug_name);
	if (!texture) {
		free(glyphs);
		return 0;
	}
...
static int load_png_bytes(AeronFontAtlas* out, AeronCommandBuffer* cmd, const uint8_t* fnt, size_t fnt_size,
						  const uint8_t* png, size_t png_size, const char* label) {
	if (!font_atlas_is_empty(out) || !cmd || !fnt || !png || !label || png_size > INT_MAX)
		return 0;
	FontMetrics metrics;
	if (!font_metrics_parse(fnt, fnt_size, label, &metrics))
		return 0;
	...
	if (!pixels || width != metrics.atlas_w || height != metrics.atlas_h) {
		Aeron_LogError("aeron.scene", "font: PNG '%s' missing or dimensions mismatch", label);
...
static int load_ktx2_bytes(AeronFontAtlas* out, AeronCommandBuffer* cmd, const uint8_t* fnt, size_t fnt_size,
						   const uint8_t* ktx_bytes, size_t ktx_size, const char* label) {
	...
	if (!ktx || ktx2_face_count(ktx) != 1 || ktx2_width(ktx) != metrics.atlas_w ||
		ktx2_height(ktx) != metrics.atlas_h) {
		Aeron_LogError("aeron.scene", "font: KTX2 '%s' missing or dimensions mismatch", label);
		ktx2_close(ktx);
		free(metrics.glyphs);
		return 0;
	}
...
static int load_files(AeronFontAtlas* out, AeronCommandBuffer* cmd, const char* basename,
					  const char* image_extension, int ktx2) {
	if (!out || !cmd || !basename)
		return 0;
	char   path[1024];
	...
	if (!fnt || snprintf(path, sizeof path, "%s.%s", basename, image_extension) >= (int)sizeof path) {
		free(fnt);
		return 0;
	}
	uint8_t*  image  = read_file(path, &image_size);
	const int result = image ? (ktx2 ? load_ktx2_bytes(out, cmd, fnt, fnt_size, image, image_size, basename)
...
static int load_vfs(AeronFontAtlas* out, AeronCommandBuffer* cmd, AeronVfs* vfs, AeronVfsRoot root,
					const char* basename, size_t maximum_file_size, const char* image_extension, int ktx2) {
	if (!out || !cmd || !vfs || !basename || !maximum_file_size)
		return 0;
	char     path[1024];
	...
	if (snprintf(path, sizeof path, "%s.fnt", basename) >= (int)sizeof path ||
		...
		return 0;
	if (snprintf(path, sizeof path, "%s.%s", basename, image_extension) >= (int)sizeof path ||
		!AeronVfs_ReadAll(vfs, root, path, maximum_file_size, &image, &image_size)) {
		free(fnt);
		return 0;
	}
	const int result = ktx2 ? load_ktx2_bytes(out, cmd, fnt, fnt_size, image, image_size, basename)
							: load_png_bytes(out, cmd, fnt, fnt_size, image, image_size, basename);
	free(image);
	free(fnt);
	return result;
...
int AeronFontAtlas_LoadVfs(AeronFontAtlas* out, AeronCommandBuffer* cmd, AeronVfs* vfs, AeronVfsRoot root,
						   const char* basename, size_t maximum_file_size) {
	return load_vfs(out, cmd, vfs, root, basename, maximum_file_size, "png", 0);
}

int AeronFontAtlas_LoadKtx2(AeronFontAtlas* out, AeronCommandBuffer* cmd, const char* basename) {
	return load_files(out, cmd, basename, "ktx2", 1);
}

int AeronFontAtlas_LoadKtx2Vfs(AeronFontAtlas* out, AeronCommandBuffer* cmd, AeronVfs* vfs, AeronVfsRoot root,
							   const char* basename, size_t maximum_file_size) {
	return load_vfs(out, cmd, vfs, root, basename, maximum_file_size, "ktx2", 1);
}
